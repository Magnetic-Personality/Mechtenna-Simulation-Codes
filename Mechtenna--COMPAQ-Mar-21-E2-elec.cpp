...
long double epsilon_0 = 8.854 * 1.0E-12  ;  //  SI units, Henries per metre   ... hyperphysics
...
bool Multiple_Angles = 0;//0;// 1;//  1;// 1; //  0; //   true;// 22nov20 set true .. false; // true;  //  0;// 0;// 2jan2019changed  true;   //    0;		//0;     //   0;    // 1;  // 0;//  true ;
...
double array_voltage_AB [100] ;	
double array_voltage_CD [100] ;	
Vector spiral_point [1000];
int sp_pts_1_turn = 21 ;
int num_sp_pts ;
Vector pA,pB ;
Vector pC,pD ;
double voltage_A, voltage_B;
double voltage_C, voltage_D;
double r_distance ;
...
//glTranslatef(  0,  -300,0);    // /25nov2020  turn ON, line 202
//gluLookAt(  0,  0,100,    0, 0, 0,    0,1,0);    // /25nov2020  turn ON, line 202
...
double dLy_spiral = dL_turns   /    (  sp_pts_1_turn - 1  )  ;
double angle_sp_pt = 2.0 * PI  /    (  sp_pts_1_turn - 1  )  ;
...
pB . x  =  turn_rad ;
pB . y  =  turn_y_last ;    // turn_y_first + .003;   //turn_y_last ;
pB . z  =  0 ;

pC . x  =  turn_rad * 2.0 ;
pC . y  =  (turn_y_first  +  turn_y_last  )   / 2.0 ;
pC . z  =  0 ;
...
// -----------------------------  points along spiral  ---------------------
kk = 1 ;
for (  ii=1  ;  ii  <=10    ;  ii++  )    //    n_turns  -  1
		{
		for (  jj=0  ;  jj  <  sp_pts_1_turn  -  1 ;  jj++  )
		{
		spiral_point [kk] .  x  =  turn_rad * std::cos (angle_sp_pt  *  jj)  ;
		spiral_point [kk] .  y  =  (ii-1)   *   dL_turns     +      (jj) * dLy_spiral        +    turn_y_first   ;
		spiral_point [kk] .  z  =  turn_rad * std::sin (angle_sp_pt  *  jj)   ;
		kk++ ;
		}
		}
		spiral_point [kk] .  x  =  turn_rad * std::cos ( 0.0 ) ;
		spiral_point [kk] .  y  = (10.0)   *   dL_turns   +    turn_y_first   ;
		spiral_point [kk] .  z  =  turn_rad * std::sin ( 0.0  ) ;
num_sp_pts = kk ;


for (  kk=1   ;   kk   <=   num_sp_pts   ;   kk++ )
{
std::cout << "sp_pts : " << kk << " "	<< spiral_point [kk] .  x << " " 
			<< spiral_point [kk] .  y << " " 
			<< spiral_point [kk] .  z << '\n'   ;
}
...
glBegin(GL_LINE_STRIP);
for (ii = 1   ;   ii   <=   num_sp_pts   ;   ii++ )     
	{
	glVertex3f(	spiral_point [ii]  .  x   * magnif ,
			spiral_point [ii]  .  y   * magnif ,
			spiral_point [ii]  .  z   * magnif   )  ;
	}
glEnd();
...
for (nn1 = 1 ; nn1 <= nx ; nn1++ )      	{     // x-direction 
for (nn2 = 1 ; nn2 <= ny ; nn2++ )   		{     // y-direction 
		
r_distance =     (  pA - meg_array_pos_UP [nn1][nn2]   ) . Modulus() ;
voltage_A +=   1.0/ (4.0 * PI * epsilon_0) * Q_cell / r_distance ;

r_distance =     (  pA - meg_array_pos_DOWN [nn1][nn2]   ) . Modulus() ;
voltage_A +=   1.0/ (4.0 * PI * epsilon_0) * Q_cell * (-1.0)  / r_distance ;


r_distance =     (  pB - meg_array_pos_UP [nn1][nn2]   ) . Modulus() ;
voltage_B +=   1.0/ (4.0 * PI * epsilon_0) * Q_cell / r_distance ;

r_distance =     (  pB - meg_array_pos_DOWN [nn1][nn2]   ) . Modulus() ;
voltage_B +=   1.0/ (4.0 * PI * epsilon_0) * Q_cell * (-1.0)  / r_distance ;


		}
		}
std::cout << "Voltage_1pt " << 1.0/ (4.0 * PI * epsilon_0) * Q_cell * (-1.0)  / r_distance  <<  '\n' ;
std::cout << "Voltage_A : " << voltage_A << '\n'  ;
std::cout << "Voltage_B : " << voltage_B << '\n'  ;
std::cout << "Difference : " << voltage_B - voltage_A  << " V" << '\n'  ;
...
//			ELECTRIC VOLTAGE  A-B ......................................
			...
			array_voltage_AB [iia] = voltage_B - voltage_A ;
			...
			r_distance =     (  pC - meg_array_pos_UP [nn1][nn2]   ) . Modulus() ;
			voltage_C +=   1.0/ (4.0 * PI * epsilon_0) * Q_cell / r_distance ;
			
			r_distance =     (  pC - meg_array_pos_DOWN [nn1][nn2]   ) . Modulus() ;
			voltage_C +=   1.0/ (4.0 * PI * epsilon_0) * Q_cell * (-1.0)  / r_distance ;
		

			r_distance =     (  pD - meg_array_pos_UP [nn1][nn2]   ) . Modulus() ;
			voltage_D +=   1.0/ (4.0 * PI * epsilon_0) * Q_cell / r_distance ;

			r_distance =     (  pD - meg_array_pos_DOWN [nn1][nn2]   ) . Modulus() ;
			voltage_D +=   1.0/ (4.0 * PI * epsilon_0) * Q_cell * (-1.0)  / r_distance ;
			}
			}
			array_voltage_CD [iia] = voltage_D - voltage_C ;
...
if   (   Multiple_Angles  ==  true   )
	...
	std::ofstream ubfile("Voltage_AB.csv")  ;
	std::ofstream ucfile("Voltage_CD.csv")  ;
	...
	if (ubfile.is_open()   )	ubfile	<<   i
					<< ','  << array_voltage_AB [i] << '\n';
		else std::cout << "the output file on disk is closed!!";

	if (ucfile.is_open()   )	ucfile	<<   i
					<< ','  << array_voltage_CD [i] << '\n';
		else std::cout << "the output file on disk is closed!!";
...
//bcc32 -Ic:\borland\BCC55\include -Lc:\borland\bcc55\lib -Lc:\borland\BCC55\psdk -Lc:\borland\bcc55\lib\psdk Mechtenna--COMPAQ-Mar-21-E2-elec.cpp
