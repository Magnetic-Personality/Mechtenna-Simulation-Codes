...
bool Multiple_Angles = 0;// 1; // 0;//  changed 11nov21;//0;//  1;//1;//     0;// 1; //true;  //  false;// 25nov20 set .. false; 

bool MAG03_display = 1; //    0;//       1;// true;    //      false;// true;   //  true; //    false;   //22nov20 set false    true   true ;    //

bool Multiple_phi = 0;//  changed 11nov211;    //0 ;    //      added 22 Jan 2021
...
double fixed_angle = 1.7*PI;// +PI/6;//  +PI/2 * 5/6 ;//+PI/6;//-PI/2;   //    -PI/6;//   0;//   -PI/6;//  0 ;//  -PI/6; //   PI/2.0; //  0; //  PI/6; // -PI/6 ; // PI/20;//  PI/2;//PI/0; //8; //    0;		//PI/8; //        PI/2   ; //   0;  // only use if we aren't in Multiple_Angle calc mode
...
long double Largest_B_component_chen = 0 ;
...
double rod=0.05 ; //.0173; 		// .0176; 		//  radius of rod in metres
			//	.0173 = .03 (rm) - .0254 (Ly)  /  2
//			put in on 15 Jan 2021, for "section of cylindrical capacitor"   ...
double a_radius = .003 ; 	// inner radius
double b_radius = 0.05  ;  //  .015 ;	// outer radius  (metres)
double L_length = 0.10; //   0.05 ; 	
double phi =  0.9*PI   ;    //  0.3333*PI;   //     0.5*PI   ;    //0.66*PI; //2.0*PI;  //    /2.0   ; //  PI/2.0 ;
double d_angle, psi;
long double Field_Component_Max = 0 ;     
double_array_arad [100]  ;
double chenx = .05 ;  // Positions for Sensor Indicator (in metres)
double cheny = .07 ;  // using Parameters of Chen Wang paper
			//  added 17 Nov 2021
...
for (jj = 0 ; jj <= 360 ; jj++ )					//  y direction
		...
		glVertex3f(   float(L_length)*magnif   ,   rod*cos(jj * PI / 180.0)  * magnif ,   rod*sin(jj * PI / 180.0)  * magnif );
		}
glEnd();
...
glVertex3f(  L_length  *magnif    ,   rod*cos(drawang * PI / 180.0)  * magnif ,   rod*sin(drawang * PI / 180.0)  * magnif );
...
// ---------------- Chen Wang Paper ... Sensor Indicator

glBegin(GL_QUADS);
glColor3f(1,0,1);
glVertex3f(   (chenx-.01)*magnif  ,    (-cheny-.01)*magnif , 0) ;
glVertex3f(   (chenx-.01)*magnif  ,    (-cheny+.01)*magnif , 0) ;
glVertex3f(   (chenx+.01)*magnif  ,    (-cheny+.01)*magnif , 0) ;
glVertex3f(   (chenx+.01)*magnif  ,    (-cheny-.01)*magnif , 0) ;
glEnd();

// ............ Chen Wang's sensor points
glColor3f(0,0,0) ;         // Black
glBegin(GL_POINTS);

for (      ii=0  ;  ii  <=  10  ;  ii++      )
	{
	glVertex3f(  (XYZ_sensor_position_chen[ii].x) * magnif,
		     (XYZ_sensor_position_chen[ii].y) * magnif,
		     (XYZ_sensor_position_chen[ii].z) * magnif   ) ;
	}
...
// long double Field_Component_Max = 0 ;  // made it "long" on 22nov2020 ... gave it VIAGRA!
//				for (ii = 0 ; ii < n_angles ; ii++ )	//  y direction
//					{
...
//  *****   Fabulous !
		...
		std::cout << "OBTAINED IN main() function ... MAX FIELD COMPONENT IS " << '\n' ;
...
//   glOrtho(-560,560,-560,560, 0, 3000);           //... 17nov 2021, expand further
     glOrtho(-690,690,-690,690, 0, 3000);           //... 30nov 2021, expand further
...
//omega = 2.0 * PI * 30;  // change on 15dec2021 166; //     2.0 * PI * 100 ;		// angular frequency
  omega = 2.0 * PI * 166; // USE omega = 166 to reproduce 
			  // Figure *** of Thesis. 166 
			  // is value used in Bickford 
			  // experiment.  30 is value used in
			  // Chen Wang Experiment.
...
Q_upper = 3.26E-4 * b_radius*phi * L_length;  // added 15dec2021
...
int iia = 0;
int ii ; 
int iiphi ;		// added 22 Jan 2021
int iiarad ;	// added 04 Feb 2021
int num_phi = 5; // 36	; 	//12;
int num_arad = 21;        //       11  ;   //  2 ;
double array_phi[100];
double array_B_Max[100];
double capac_per_m ;
double epsilon_0  =  8.854E-12 ;  	//  F  m-1     from Hyperphysics
double epsilon_r  =  1.0000000 ;  	//
double Voltage = 2000   ;
double psi2 ;

//   for (iiphi=1 ; iiphi <= num_phi ; iiphi ++ ) 
//     for (iiarad=1 ; iiarad <= num_arad ; iiarad ++ )   //  added 4 Feb 2021

//	{
//		phi =               2.0 * PI    /    double(  num_phi  )    * double(iiphi)  ;      
		//phi = PI ;	//   use what previously was found
			//   to yield the Maximum Field     ............ 4 Feb 2021

		//   previous simulations had     a_radius = .003   ,   b_radius = .015
		//   have a_radius go from .003 up to .013  (metres)

//		a_radius =  .003  +   (iiarad-1) * (.010 / double(num_arad-1)   )   ;


		// eqn:  C/L = (2pi e_r e_0 ) / ( ln [b/a]) 
		// Q = CV ...  
		
		capac_per_m = 2.0 * PI * epsilon_r * epsilon_0   /    std::log(  b_radius / a_radius  )  ;
		
		//Q_upper = capac_per_m   *   L_length   *   Voltage ;        // L_length defined in preamble
	
		//Q_upper = 3.46E-9 * phi / (2.0*PI); 
		//Q_cell = Q_upper / (nx * ny) ;
		Field_Component_Max = 0;
...
//						// yd term added 25 nov 2020
		...
		psi2 = - phi / 2.0   + angle       +    0.5 * d_angle    +     (nn2-1) * d_angle    ;
...
//						// yd term added 25 nov 2020
		...
		Cell_centre_INNER =      	     i_unit * (  (nn1-1) * Lx2 + 0.5 * Lx2  )
					+  j_unit *  (+1.0) * b_radius  *  (  cos(psi2) )
					+  k_unit *  (+1.0) * b_radius  *  (  sin(psi2) )      ;

					// above changed j_unit * (-1.0)  to become
					//  j_unit * (+1.0) ... 15Nov2021
					//  also changed a_radius to b_radius
...
//   5 Feb 2021 ... remove above line, forgot reason for above line, test w/ just 1 sign charge perhaps ?
...
// add this next loop on 17 Nov 2021
// this is for the Chen Wang sensor 
		
for (iiq = 0   ;   iiq <= 10   ;   iiq++   )  // 11 not 10 values
	{
		XYZ_sensor_position_chen[iiq] =  i_unit * ( chenx )
						+ j_unit *    (   (-1.0)*cheny  + 0.01     
						         + iiq * (-0.01)		)
						+ k_unit * 0
						;		
				obs_point = XYZ_sensor_position_chen[iiq] ;
					B_up_from_2by2 = Calc_B_from_2by2_THICK_UPPER(obs_point) ; 	// function call
					B_lo_from_2by2 = Calc_B_from_2by2_THICK_LOWER(obs_point) ; 	// function call
							//   Vector (0,0,0);   //  17jan2019      
				XYZ_sensor_fields_chen[iiq] = (  B_up_from_2by2 + B_lo_from_2by2  ) ;
					if (   XYZ_sensor_fields_chen[iiq].Modulus()     >    Largest_B_component_chen  	) 
						{
						Largest_B_component_chen = XYZ_sensor_fields_chen[iiq].Modulus()    ;    // 17nov 2021
						}
	}			
std::cout << " ----chen fields calculated " << '\n'  ;
...
std::cout << fabs(Y_sensor_avg_many_angles[ii] )   << "\n" ;
	...
	//array_B_Max[iiphi]   =   Field_Component_Max   ;		
//	array_B_Max[iiarad]   =   Field_Component_Max   ;
	
//std::cout << "From MAIN LOOP:  MAX FIELD COMPONENT IS " << '\n' ;
//		std::cout << Field_Component_Max << " tesla " << '\n' ;

// if (Multiple_phi == false) 	break ;
 

//	}  	// end    iiphi  loop   .... ***     ***  4 Feb 2021 changed to iiarad loop  ***     *** 
...
std::ofstream uafile("magnetic-6-feb-2021.csv")  ;

// for some reason we need std::   before ofstream or gives error.  Need before cout below also ....
//  22 Jan 2021

//   if (ufile.is_open()) ufile << "TimeStep" << "," << "Pop" << "," << "Cohr" << '\n';         

//for ( int jj = 1   ;   jj <= num_phi   ;   jj++ )
for ( int jj = 1   ;   jj <= num_arad   ;   jj++ )
		{
		if (uafile.is_open()   )	uafile	<<    2.0 * PI    /    double(  num_phi  )    * double(jj)
					<< ','  << array_B_Max[jj]
					<<  '\n' ;
...
std::cout << "ZAPPA ZAPPA ZAPPA ! !  "	<< '\n' ;		
	
	// .............. get the MAX / MIN ---3axes .....................		
	//double Field_3axes_Min, Field_3axes_Max, Field_3axes;
	//	Field_3axes_Min =   XYZ_sensor_fields_chen[0].Modulus() ;
	//	Field_3axes_Max = 0;
	...
	for (iq = 0 ; iq <= num_sensor_fields_minus_one ; iq++)
	{
		...
		// ......[iq].... in previous function   ???
		// .........................................
		Field_3axes =	XYZ_sensor_fields_chen[iq].Modulus() ;
			if (   Field_3axes  >   Field_3axes_Max )  Field_3axes_Max = Field_3axes ;
			if (   Field_3axes  <   Field_3axes_Min )  Field_3axes_Min = Field_3axes ;
		}


//long double ppz,  Bzr,  Bzrb  ;
...
for (iq = 0 ; iq <= num_sensor_fields_minus_one ; iq++)
{
ppx = XYZ_sensor_position_chen[iq]  .  x  * magnif ;
ppy = XYZ_sensor_position_chen[iq]  .  y  * magnif ;
ppz = XYZ_sensor_position_chen[iq]  .  z  * magnif ;


Bxr = XYZ_sensor_fields_chen [iq] .  x ;
Byr = XYZ_sensor_fields_chen [iq] .  y ;
Bzr = XYZ_sensor_fields_chen [iq] .  z ;
...
if (sensor_textout == true)
	{
		std::cout << "CHEN sensor [" << iq << "] : " << 
		XYZ_sensor_fields_chen [iq] .  x  <<  " "  <<
		XYZ_sensor_fields_chen [iq] .  y  <<  " "  <<
		XYZ_sensor_fields_chen [iq] .  z  <<  " "  <<	'\n'  ;
	}
...
//bcc32 -Ie:\borland\BCC55\include -Le:\borland\bcc55\lib -Le:\borland\BCC55\psdk -Le:\borland\bcc55\lib\psdk Mechtenna-cylinder-B2-multiple-phi-B-nov21.cpp
//bcc32 -Ic:\borland\BCC55\include -Lc:\borland\bcc55\lib -Lc:\borland\BCC55\psdk -Lc:\borland\bcc55\lib\psdk Mechtenna-cylinder-B2-multiple-phi-B-nov21-2c.cpp
