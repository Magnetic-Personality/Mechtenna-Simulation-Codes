...
int nWinx=800 ; int nWiny=700;  //700; //660 ;
...
long double Lx = .0254, Ly = .0254, Lz = .0254  ;     	// Electret Length, Width in metres
						//	added Lz on 19 Mar 2019
...
Vector Calc_B_xy_wire (Vector) ;	// added 19 mar 2019
Vector Calc_B_xz_wire (Vector) ;	// added 19 mar 2019

double I_current =   1.0E-6   ;   // start out with a microampere of current in the wire loop
...
long double Rect_obs_z = + 0 ;   //   0.03 ;   //0;
long double Rect_obs_x2 = +.05 ; // -.01;  //+.05;     //-.01;  //+.05;     //-.01;  // +.05;     // -.01;  //
...
bool Multiple_Angles = false; // true;  //  0;// 0;// 2jan2019changed  true;   //    0;		//0;     //   0;    // 1;  // 0;//  true ;
bool Multiple_Phases = 1;//false;  //true; // true; 
bool MAG03_display = 1;//false;// true ; // false; //true ; //   false; //  true ;    //
...
double fixed_angle = PI/20;//  PI/2;//PI/0; //8; //    0;		//PI/8; //        PI/2   ; //   0;  // only use if we aren't in Multiple_Angle calc mode

long double h = .000250  ;         // electret height (thickess) in metres
...
double Y_sensor_avg_many_phases [30];
double Z_sensor_avg_many_phases [30];
double X_sensor_avg_many_phases [30];
...
//	 	gluLookAt(  -300 , -300 , 270,    0, 0, 0,    0,0,1);
...
//  ........ Wire Loop in xz plane ........

glColor3f(1,0.7,.3);
glBegin(GL_LINE_LOOP);
glVertex3f(  0, 0,  -Lz/2 * magnif) ;
glVertex3f(  Lx * magnif, 0, -Lz/2 * magnif) ;
glVertex3f(  Lx * magnif, 0, +Lz/2 * magnif) ;
glVertex3f(  0, 0,  +Lz/2  * magnif) ;
glEnd();
...
//		<< array_velocity_UP [jj].z << " " << '\n' ;
	...
	glColor3f(.7,.7,0) ;		// yellow ... no,  .7 .7 0 is murky green
			glVertex3f( magnif  *  meg_array_pos_UP[5][jj]  .x , 0 , 0 );   // changed 14-dec-2018
			glVertex3f(px,py,pz);
...
//	if (Multiple_Angles == true)  
	...
	//		glLineWidth(1);			// added 9 Jan 2019
			...
			glEnd();
				...
				glBegin(GL_LINES);
				for (ii = 0 ; ii < n_phases ; ii++ )	//  y direction
					{		
						glVertex3f(  ii * 100 / (n_phases-1) , -5 , 0);		
						glVertex3f(  ii * 100 / (n_phases-1) , +5 , 0);		
					}
				glEnd();

double Field_Component_Max = 0 ;
				for (ii = 0 ; ii < n_phases ; ii++ )	//  y direction
					{
					if ( std::fabs(Y_sensor_avg_many_phases[ii] ) > Field_Component_Max) Field_Component_Max = std::fabs(Y_sensor_avg_many_phases[ii] )  ;
					if ( std::fabs(Z_sensor_avg_many_phases[ii] ) > Field_Component_Max) Field_Component_Max = std::fabs(Z_sensor_avg_many_phases[ii] )  ;
					if ( std::fabs(X_sensor_avg_many_phases[ii] ) > Field_Component_Max) Field_Component_Max = std::fabs(X_sensor_avg_many_phases[ii] )  ;
					}
		...
		if (Field_Component_Max == 0) 
		{
			...
			std::exit(0) ;
				...
				glBegin(GL_LINE_STRIP);
				for (ii = 0 ; ii < n_phases ; ii++ )	
					{		
						glVertex2f(  ii * 100 / (n_phases-1) , Z_sensor_avg_many_phases[ii] / Field_Component_Max * 100);				
					}
				glEnd();
			...
			glColor3f(0,1,0);   // green for Y_sensor_field_component
				...
				for (ii = 0 ; ii < n_phases ; ii++ )	
					{		
						glVertex2f(  ii * 100 / (n_phases-1) , Y_sensor_avg_many_phases[ii]  / Field_Component_Max * 100);				
					}
				glEnd();
			...
			glColor3f(1,0,0);   // red for X_sensor_field_component 
				...
				for (ii = 0 ; ii < n_phases ; ii++ )	
					{		
						glVertex2f(  ii * 100 / (n_phases-1) , X_sensor_avg_many_phases[ii]   / Field_Component_Max * 100);				
					}
				glEnd();
...
//    was -170 170 -170 170
//glOrtho(-210,210,-210,210, 0, 650);
glOrtho(-210,280,-210,210, 0, 790);
...
for (iia=0 ; iia < n_phases ; iia++ ) 
	{
//iia = 0; 
	angle = (1.0 * PI) / double(n_angles-1) * double(iia)  ;
	phase = (1.0 * PI) / double(n_phases-1) * double(iia)  ;
	//   angle = PI/5		;//
	Angle_Array[iia] = angle ;
	Phase_Array[iia] = angle ;
...
Vector B_from_xy_wire;
Vector B_from_xz_wire;
...
//std::exit(0);
	...
	//Field_Value[nfx][nfy] = Calc_B_xy_wire(obs_point)  + Calc_B_xz_wire(obs_point)  ;
	Field_Value[nfx][nfy] =     Calc_B_xy_wire(obs_point) + Calc_B_xz_wire(obs_point)  ;
	...
	}   // end         if  MAG03_display == false		  braces   ******************
	...
	if ( Multiple_Phases == true )
			{
//			Field_Phase_Loop [iia] = Field_Value[1][1] ; 
			}
...
//											;		
					...
					obs_point = Y_sensor_position[iiq] ;
						...
						B_from_xy_wire = Calc_B_xy_wire(obs_point) ;   //* sin(phase) ;
						B_from_xz_wire = Calc_B_xz_wire(obs_point) ;   //* cos(phase) ;

						//B_up_from_2by2 = Calc_B_from_2by2_THICK_UPPER(obs_point) ; 	// function call
						//B_lo_from_2by2 =  Calc_B_from_2by2_THICK_LOWER(obs_point) ; 	// function call
									//  Vector (0,0,0);   //  17jan2019 
			if (  Multiple_Phases == false  )		Y_sensor_fields[iiq] = B_from_xy_wire + B_from_xz_wire ;			// 
			if (  Multiple_Phases == true  )		Y_sensor_fields[iiq] = 
													B_from_xy_wire * sin(phase)   +   B_from_xz_wire * cos(phase);	


						if (std::fabs(Y_sensor_fields[iiq].y) > Largest_B_component  	) Largest_B_component = std::fabs(Y_sensor_fields[iiq].y)  ;  // 3jan2019... changed .z to .y (component change)
					}	
				// ................. Z Sensor ................
				for (iiq = 0   ;   iiq <= 10   ;   iiq++   )  // 11 not 10 values
					{
					...
					obs_point = Z_sensor_position[iiq] ;
						...
						B_from_xy_wire = Calc_B_xy_wire(obs_point) ;
						B_from_xz_wire = Calc_B_xz_wire(obs_point) ;

						// B_up_from_2by2 = Calc_B_from_2by2_THICK_UPPER(obs_point) ; 	// function call
						// B_lo_from_2by2 = 		 Calc_B_from_2by2_THICK_LOWER(obs_point) ; 	// function call
									//    Vector (0,0,0);   //  17jan2019
			if (  Multiple_Phases == false  )		Z_sensor_fields[iiq] = B_from_xy_wire + B_from_xz_wire ;			// 
			if (  Multiple_Phases == true   )		Z_sensor_fields[iiq] = 
													B_from_xy_wire * sin(phase)   +   B_from_xz_wire * cos(phase);	



						if (std::fabs(Z_sensor_fields[iiq].x) > Largest_B_component  	) Largest_B_component = std::fabs(Z_sensor_fields[iiq].x)  ;
				...
				for (iiq = 0   ;   iiq <= 10   ;   iiq++   )  // 11 not 10 values
					...
					// these next lines which give command
					// to calculate the B-field ..... are
					// the ones where we need to make the
					// change to just calculate the B-field
					// due to the WIRE LOOPS ... 19mar2019
					Vector B_up_from_2by2;
					Vector B_lo_from_2by2;
		...
		if (  Multiple_Phases == false  )		X_sensor_fields[iiq] = B_from_xy_wire + B_from_xz_wire ;		
		if (  Multiple_Phases == true  )		X_sensor_fields[iiq] = 
													B_from_xy_wire * sin(phase)   +   B_from_xz_wire * cos(phase);	

					if (std::fabs(X_sensor_fields[iiq].z) > Largest_B_component  	) Largest_B_component = std::fabs(X_sensor_fields[iiq].z)  ;    //3jan19  .y changed to .z
		...
		Y_sensor_avg_many_phases[iia] = Y_sensor_avg;
		Z_sensor_avg_many_phases[iia] = Z_sensor_avg;
		X_sensor_avg_many_phases[iia] = X_sensor_avg;
...
//	if (Multiple_Angles == false  ) break;	// break out of iia loop if we only are doing 1 angle
	if (Multiple_Phases == false  )	break;	// break out of iia loop if we are not simulating 2 out-of-phase coils	
...
// ............. Console Output of field values for the set of Angles
		...
		if (Multiple_Phases == true)
			...
			std::cout << '\n' ;
				...
				for (int ii = 0 ; ii < n_phases ; ii++)
				{	
				std::cout << "ii: " << ii << "  Phase: " << Phase_Array[ii] << "  Sensor YZX: " << 
				Y_sensor_avg_many_phases [ii] << " " << 
				Z_sensor_avg_many_phases [ii] << " " << 
				X_sensor_avg_many_phases [ii] << " " << 
				'\n' ;
				}
			}	
...
{
int ix,iy ;
long double ppx, ppy, ppz, Bxr, Byr, Bxrb, Byrb  ;
...
ppz = Field_Position[ix][iy]  .  z  * magnif ;

Bxr = Field_Value [ix] [iy]  .  x ;
Byr = Field_Value [ix] [iy]  .  y ;

std::cout << "Bxr^2 + Byr^2 : " << pow(Bxr,2) + pow(Byr,2)   << '\n'  ;

if ( Bxr*Bxr + Byr*Byr )  
							{		
							//	std::cout << "Bxr^2 +Byr^2 = 0 " << '\n'  ;		
							}
...
Vector Calc_B_xy_wire(Vector obs_point_fn) 
{
int fnn1, fnn2   ;
int nxw = 20, nyw = 20   ;
double LLy2, LLx2  ;				// cannot use Ly2, Ly2 defined in main() .... cannot be accessed here
LLy2 = Ly / nyw ;
LLx2 = Ly / nxw ;
Vector B_total_xy (0,0,0);
Vector dB_xy (0,0,0) ;

Vector r_vec ;
Vector r_hat ;

Vector delta_S (0,0,0) ;
Vector Element_Pos (0,0,0) ;

// .... first do side of coil along y-axis and opposite direction to y-axis ....          
// .... remember I_current is a Global Variable ....

for (fnn2 = 1 ; fnn2 <= nyw ; fnn2++ )						// --------------   <1>
		{
			delta_S =  j_unit * (-1.0) * ( Ly / nyw )     ;     //  Vector * double
			Element_Pos.y  =  +Ly / 2.0   -   (fnn2-1) * LLy2 - 0.5 * LLy2 ;

			r_vec = Element_Pos	- obs_point_fn  ;
			r_hat = r_vec.Hat()   ;

			dB_xy =  r_hat * delta_S * (-1.0 * I_current )		// use IdS = qv
					/ std::pow(   r_vec.Modulus()  ,  2 )   *  ( mu_0   /   ( 4 * PI ) ) ;			
		
			// bring in the u0/4pi later .....   NO, do it above!
			B_total_xy  =  B_total_xy  +  dB_xy ;
		}

	//  side of coil coincident and opposite direction to y-axis done ...
	//  next, do side of coil at x = Lx, same direction as y-axis  ...

			for (fnn2 = 1 ; fnn2 <= nyw ; fnn2++ )			// --------------   <2>
		{
			delta_S =  j_unit * (+1.0) * ( Ly / nyw )     ;     //  Vector * double
	...
	//   next do current in  4th quadrant, y = -Ly/2, current in +x direction 

			for (fnn1 = 1 ; fnn1 <= nxw ; fnn1++ ) 			// --------------   <3>   
		{
			delta_S =  i_unit * (+1.0) * ( Lx / nxw )     ;     //  Vector * double
			Element_Pos.x  =  0   +   (fnn1-1) * LLx2 + 0.5 * LLx2 ;
			Element_Pos.y  =  -Ly / 2.0 ;
			r_vec = Element_Pos	- obs_point_fn  ;
			r_hat = r_vec.Hat()   ;
			...
			//   next do current in  1st quadrant, y = +Ly/2, current in -x direction 

			for (fnn1 = 1 ; fnn1 <= nxw ; fnn1++ )				// --------------   <4>
		{
			delta_S =  i_unit * (-1.0) * ( Lx / nxw )     ;     //  Vector * double
			Element_Pos.x  =  Lx   -   (fnn1-1) * LLx2 - 0.5 * LLx2 ;
			Element_Pos.y  =  Ly / 2.0 ;
			r_vec = Element_Pos	- obs_point_fn  ;
			r_hat = r_vec.Hat()   ;
...
Vector Calc_B_xz_wire(Vector obs_point_fn) 
{
	// Vector B_total_xz (0,0,0);

int fnn1, fnn2 ,fjj  ;
int nxw = 20, nzw = 20   ;
double LLz2 , LLx2  ;				// cannot use Ly2, Ly2 defined in main() .... cannot be accessed here
LLz2 = Lz / nzw ;
LLx2 = Ly / nxw ;
Vector B_total_xz (0,0,0);
Vector dB_xz (0,0,0) ;
...
// .... first do side of coil along z-axis and opposite direction to z-axis ....          
// .... remember I_current is a Global Variable ....

for (fjj = 1 ; fjj <= nzw ; fjj ++ )						// --------------   <a>
		{
			delta_S =  k_unit * (-1.0) * ( Lz / nzw )     ;     //  Vector * double
			Element_Pos.z  =  +Lz / 2.0   -   (fjj-1) * LLz2 - 0.5 * LLz2 ;

			r_vec = Element_Pos	- obs_point_fn  ;
			r_hat = r_vec.Hat()   ;

			dB_xz =  r_hat * delta_S * (-1.0 * I_current )		// use IdS = qv
					/ std::pow(   r_vec.Modulus()  ,  2 )   *  ( mu_0   /   ( 4 * PI ) ) ;			
		
			// bring in the u0/4pi later .....   NO, do it above!
			B_total_xz  =  B_total_xz  +  dB_xz ;
		}

	//  side of coil coincident and opposite direction to z-axis done ...
	//  next, do side of coil at x = Lx, same direction as z-axis  ...

			for (fjj = 1 ; fjj <= nzw ; fjj++ )			// --------------   <b>
		{
			delta_S =  k_unit * (+1.0) * ( Lz / nzw )     ;     //  Vector * double
	...
	//   next do current at   z = -Lz/2   current in +x direction 

			for (fjj = 1 ; fjj <= nxw ; fjj++ ) 			// --------------   <c>   
		{
			delta_S =  i_unit * (+1.0) * ( Lx / nxw )     ;     //  Vector * double
			Element_Pos.x  =  0   +   (fjj-1) * LLx2 + 0.5 * LLx2 ;
			Element_Pos.z  =  -Lz / 2.0 ;
			...
			for (fjj = 1 ; fjj <= nxw ; fjj++ )				// --------------   <d>
		{
			delta_S =  i_unit * (-1.0) * ( Lx / nxw )     ;     //  Vector * double
			Element_Pos.x  =  Lx   -   (fjj-1) * LLx2 - 0.5 * LLx2 ;
			Element_Pos.z  =  Lz / 2.0 ;
...
//bcc32 -Ic:\borland\BCC55\include -Lc:\borland\bcc55\lib -Lc:\borland\BCC55\psdk -Lc:\borland\bcc55\lib\psdk Source1-Mechtenna-COMPAQ.cpp
