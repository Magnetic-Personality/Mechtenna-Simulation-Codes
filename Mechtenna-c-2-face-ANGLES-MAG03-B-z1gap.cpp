...
bool Multiple_Angles = 0;//  true;// false; // true;  //  0;// 0;// 2jan2019changed  true;   //    0;		//0;     //   0;    // 1;  // 0;//  true ;
bool MAG03_display = true;//   false;// true ;    //
bool sensor_textout = true;

bool multiple_z1_gap = 0;//  true;// false; //  true; // false; // true; // false; //true;   //  false;  //  line added Jan 14, 2019   (remember Woodley Auguste)
...
//9jan	 	gluLookAt(  -300 , -300 , 270,    0, 0, 0,    0,0,1);
...
double Field_Component_Max = 0 ;
				for (ii = 0 ; ii < n_angles ; ii++ )	//  y direction
					{
					if ( std::fabs(Y_sensor_avg_many_angles[ii] ) > Field_Component_Max) Field_Component_Max = std::fabs(Y_sensor_avg_many_angles[ii] )  ;
					if ( std::fabs(Z_sensor_avg_many_angles[ii] ) > Field_Component_Max) Field_Component_Max = std::fabs(Z_sensor_avg_many_angles[ii] )  ;
					if ( std::fabs(X_sensor_avg_many_angles[ii] ) > Field_Component_Max) Field_Component_Max = std::fabs(X_sensor_avg_many_angles[ii] )  ;
					}
...
Array_z1_gap[0] = 0.005 ;  // this is 0.5 cm, like in Draper slides and our earlier simulation run
for (i=1; i < num_z1_gaps ; i++)    // if num_z1_gaps is 11, i = 1,2,..,10 
	{
		Array_z1_gap[i] = double(i) * 0.01;  // 1 * .01 metres
											 // (2,3,4) * .01 metres 
											 // = 1, 2, 3, 4 cm
	}
...
int iiz ;
for (iiz = 0 ; iiz < num_z1_gaps ; iiz ++ )
{


if (multiple_z1_gap == true)		
								{
									n_angles = 3;	// do 0, PI/2, PI
								}
...
// ... lines for Field Array in 2D rectangle removed ...
...
//											;		
	...
	}	// end iia loop  which started around line 719  ... 490 to 719 changed on 14-jan-2019

	if (multiple_z1_gap == true )
	{
X_multiple_z1_gap[iiz] = X_sensor_avg_many_angles[0] ;  // at angle = 0 radians
Y_multiple_z1_gap[iiz] = Y_sensor_avg_many_angles[1] ;  // at angle = PI/2 radians
Z_multiple_z1_gap[iiz] = Z_sensor_avg_many_angles[1] ;  // at angle = PI/2 radians
	}

	if (multiple_z1_gap == false ) break;

	}	// end iiz loop ... iiz is to go through multiple z1_gap values ...
...
// ............. Console Output of field values for the set of Angles
		...
		if (multiple_z1_gap == true)
			{
			std::cout << '\n' ;
			std::cout << "Multiple z1 gaps ::::::: "<< '\n' ;
			std::cout << "z1_gap, Y, Z, X : "<< '\n' ;

			for (int ii = 0 ; ii < num_z1_gaps ; ii++)
				{	
					std::cout << Array_z1_gap[ii] << '\t' <<
				Y_multiple_z1_gap[ii]  << " " << 
				Z_multiple_z1_gap[ii]  << " " << 
				X_multiple_z1_gap[ii]  << " " << 
				'\n' ;
				}
			
		//  next, write it out to a FILE .....
		

		std::ofstream tfile("file-4-output.csv");

if (tfile.is_open()  ) tfile << "Multiple z1 gaps ::::::: "<< '\n' 
			<< "z1_gap, Ydist, Y_field, Zdist, Z_field, Xdist, X_field : " << '\n' ;

for (int ii = 0 ; ii < num_z1_gaps ; ii++)
				{	
if (tfile.is_open()  ) tfile << Array_z1_gap[ii] << ',' 
	<< Array_z1_gap[ii] + DW_y <<  "," <<   std::fabs(Y_multiple_z1_gap[ii] )  << "," 
	<< Array_z1_gap[ii] + DW_z <<  "," <<	std::fabs(Z_multiple_z1_gap[ii] ) << "," 
	<< Array_z1_gap[ii] + DW_x <<  "," <<	std::fabs(X_multiple_z1_gap[ii] ) << 
				'\n' ;
				
		else std::cout << "the POSITIONS file is closed!!";
				}
tfile.close();
