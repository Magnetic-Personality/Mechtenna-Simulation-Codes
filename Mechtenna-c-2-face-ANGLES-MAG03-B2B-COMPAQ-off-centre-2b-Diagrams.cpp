...
bool Multiple_Angles = 0;//  1;//1;//     0;// 1; //true;  //  false;// 25nov20 set .. false; 

bool MAG03_display = 1;// true;    //      false;// true;   //  true; //    false;   //22nov20 set false    true   true ;    //
//25nov20 set false 
...
double fixed_angle = PI/6; // -PI/6 ; // PI/20;//  PI/2;//PI/0; //8; //    0;		//PI/8; //        PI/2   ; //   0;  // only use if we aren't in Multiple_Angle calc mode
...
double rm = 0.03;       //16;//     14;//03; //   0.040 ;
double rmag = 0.03;    //  0.16;//      14;//03 ; 		//  added 27 nov 2020
double rod=.0176; 		//  radius of rod
...
//    was -170 170 -170 170
//  glOrtho(-210,210,-210,210, 0, 650);       ... 2dec 2020  expand viewscreen by 1.5
 //  glOrtho(-315,315,-315,315, 0, 975);      ... or by 2.0
glOrtho(-420,420,-420,420, 0,1100);
...
for (nn1 = 1 ; nn1 <= nx ; nn1++ )        //  1 to 100 inclusive  (initially)
	...
	if (nn1 == x_row_num) 
		...
		r_from_axis = std::sqrt (       // r from axis of ELECTRET, not from y-axis ,,,,,  25nov
			std::pow(0.0 , 2)	+
			std::pow(Cell_centre_UP.y    -    yd    , 2) +  		 // -yd ... remove the yd as if we shift the electret back to being centered on the y-axis  25 Nov 2020
			std::pow(Cell_centre_UP.z , 2)
				) ;
		speed = r_from_axis * omega  ;
		
		yz_vec.x = 0 ;
		yz_vec.y = Cell_centre_UP.y   -  yd   ;       //  don't add 'yd' this time
...
//	<< "\n" ;
						...
						std::pow(0.0 , 2)	+
						std::pow(Cell_centre_DOWN.y     -    yd    , 2) +       // +yd  25 Nov 2020
						...
						yz_vec.x = 0 ;
						yz_vec.y = Cell_centre_DOWN.y   -    yd   ;
...
//bcc32 -Ic:\borland\BCC55\include -Lc:\borland\bcc55\lib -Lc:\borland\BCC55\psdk -Lc:\borland\bcc55\lib\psdk Mechtenna-c-2-face-ANGLES-MAG03-B2B-COMPAQ-off-centre-2b-Diagrams.cpp
